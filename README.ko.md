...
Chill Codex는 자신이 시작한 실행 명령만 제어할 수 있습니다. Codex가 공식 앱 안에서 업데이트된 뒤 스스로 다시 실행되면, 다시 실행된 프로세스에는 `--force-prefers-reduced-motion`이 유지되지 않을 수 있습니다.

이 경우 Codex를 완전히 종료한 뒤 `~/Applications/Chill Codex.app`을 다시 여세요. [동작 확인](#동작-확인)의 명령으로 플래그가 붙어 있는지 확인할 수 있습니다.
