...
Chill Codex が制御できるのは、自分自身が開始した起動コマンドだけです。Codex が公式アプリ内で更新され、そのまま再起動した場合、再起動後のプロセスには `--force-prefers-reduced-motion` が引き継がれないことがあります。

この場合は Codex を完全に終了し、`~/Applications/Chill Codex.app` をもう一度開いてください。[動作確認](#動作確認) のコマンドでフラグが付いているか確認できます。
